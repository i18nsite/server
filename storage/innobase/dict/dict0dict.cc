...
dict_table_t *dict_sys_t::acquire_temporary_table(table_id_t id) const noexcept
{
  ...
  for (dict_table_t *t= static_cast<dict_table_t*>(temp_id_hash.cell_get
                                                   (ut_fold_ull(id))->node); t;
       t= t->id_hash)
  {
...
{
  ut_ad(frozen());
  for (dict_table_t *t= static_cast<dict_table_t*>(table_id_hash.cell_get
                                                   (ut_fold_ull(id))->node); t;
       t= t->id_hash)
  {
    ut_ad(!t->is_temporary());
  ...
  const noexcept
{
  ...
  for (dict_table_t *table= static_cast<dict_table_t*>
       (table_hash.cell_get(my_crc32c(0, name.data(), name.size()))->node);
...
inline void dict_sys_t::add(dict_table_t *table) noexcept
{
  ...
  after= reinterpret_cast<dict_table_t**>
    (&table_hash.cell_get(my_crc32c(0, table->name.m_name,
                                    strlen(table->name.m_name)))->node);
  for (; *after; after = &(*after)->name_hash)
  {
    ut_ad((*after)->cached);
    ut_a(strcmp((*after)->name.m_name, table->name.m_name));
  }
  *after= table;
