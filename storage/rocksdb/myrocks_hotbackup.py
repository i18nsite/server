...
class MiscFilesProcessor:
    ...
    def check_frm_timestamp(self, fname, path):
        if not self.skip_check_frm_timestamp and fname.endswith(b".frm"):
            if os.path.getmtime(path) > self.start_backup_time:
                logger.error(
                    "FRM file %s was updated after starting backups. "
                    "Schema could have changed and the resulting copy may "
                    "not be valid. Aborting. "
                    "(backup time: %s, file modifled time: %s)",
                    path,
                    datetime.datetime.fromtimestamp(self.start_backup_time).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                    datetime.datetime.fromtimestamp(os.path.getmtime(path)).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    ),
                )
    ...
    def get_databases(self):
        ...
        for db in dirs:
            if (
                not db.startswith(b".")
                and not self._is_socket(db)
                and not db == b"#rocksdb"
            ):
                dbs.append(db)
        return dbs
...
class MySQLBackup(MiscFilesProcessor):
    ...
    def __init__(self, datadir, writer, skip_check_frm_timestamp, start_backup_time):
        MiscFilesProcessor.__init__(
            self, datadir, skip_check_frm_timestamp, start_backup_time
        )
        self.writer = writer
...
class MiscFilesLinkCreator(MiscFilesProcessor):
    ...
    def __init__(
        self, datadir, snapshot_dir, skip_check_frm_timestamp, start_backup_time
    ):
        MiscFilesProcessor.__init__(
            self, datadir, skip_check_frm_timestamp, start_backup_time
        )
...
class RocksDBBackup:
    ...
    def init_target_files(self, prev):
        ...
        for f in os.listdir(self.source_dir):
            if f.endswith(rocksdb_data_suffix):
                ...
                if (
                    prev is not None
                    and f in prev.total_sent_sst
                    and int(os.stat(f).st_size) == prev.total_sent_sst[f]
                ):
                    continue
    ...
    def do_backup_until(self, time_limit):
        logger.info(
            "Starting backup from snapshot: target files %d", len(self.target_sst)
        )
        ...
        for fname, size in self.target_sst.items():
            ...
            if elapsed_seconds > time_limit and self.has_sent_all_sst() is False:
                logger.info(
                    "Snapshot round finished. Elapsed Time: %5.2f.  Remaining sst files: %d",
                    elapsed_seconds,
                    len(self.target_sst) - len(self.sent_sst),
                )
                self.do_cleanup()
                break
    ...
    def should_show_progress(self, size):
        if int(self.total_sent_sst_size / self.show_progress_size_interval) > int(
            (self.total_sent_sst_size - size) / self.show_progress_size_interval
        ):
            return True
        else:
            return False
    ...
    def show_progress(self, size, seconds):
        logger.info(
            "Backup Progress: %5.2f%%   Sent %6.2f GB of %6.2f GB data, Transfer Speed: %6.2f MB/s",
    ...
    def print_backup_report(self):
        logger.info(
            "Sent %6.2f GB of sst files, %d files in total.",
            self.total_sent_sst_size / 1024 / 1024 / 1024,
...
class MySQLUtil:
    ...
    def connect(user, password, port, socket=None):
        if socket:
            dbh = MySQLdb.Connect(user=user, passwd=password, unix_socket=socket)
        else:
            dbh = MySQLdb.Connect(
                user=user, passwd=password, port=port, host="127.0.0.1"
            )
        return dbh

    @staticmethod
    def create_checkpoint(dbh, checkpoint_dir):
        sql = "SET GLOBAL rocksdb_create_checkpoint='{0}'".format(checkpoint_dir)
        cur = dbh.cursor()
        cur.execute(sql)
        cur.close()
...
class BackupRunner:
    ...
    def start_backup_round(self, backup_round, prev_backup):
        ...
        try:
            ...
            dbh = MySQLUtil.connect(
                opts.mysql_user, opts.mysql_password, opts.mysql_port, opts.mysql_socket
            )
    ...
    def backup_mysql(self):
        try:
            ...
            if opts.output_stream:
                ...
                raise Exception("Currently only streaming backup is supported.")
            b = MySQLBackup(
                self.datadir, w, opts.skip_check_frm_timestamp, self.start_backup_time
            )
...
class WDTBackup:
    ...
    def backup_with_timeout(self, backup_round):
        ...
        try:
            ...
            link_creator = MiscFilesLinkCreator(
                self.datadir,
                snapshot_dir,
            ...
            remote_process = subprocess.Popen(
                remote_cmd.split(), stdout=subprocess.PIPE, stderr=server_log
            )
...
backup_wdt_usage = (
    "Backup using WDT: myrocks_hotbackup "
...
def move_back():
    if (
        opts.rocksdb_datadir is None
        or opts.rocksdb_waldir is None
        or opts.backupdir is None
        or opts.datadir is None
    ):
